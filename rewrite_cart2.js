...
import { Store, ShoppingBag, Truck, Navigation, MapPin } from 'lucide-react';
...
export default function Cart() {
  ...
  useEffect(() => {
    if (user) {
      supabase.from('customer').select('address, phone, latitude, longitude, distance').eq('id', user.id).single().then(({ data }) => {
        if (data) {
          if (data.address) {
          ...
          if (data.latitude && data.longitude) {
            ...
            if (data.distance !== null && data.distance !== undefined) {
              ...
              setDrivingDistanceKm(data.distance);
              
              // Automatically lock the map and calc fee without calling Ola API
              setIsCalculated(true);
              calculateFeeOnly(data.distance);
            }
          }
        }
      });
