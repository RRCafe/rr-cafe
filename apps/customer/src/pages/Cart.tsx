...
export default function Cart() {
  ...
  const [initialPin, setInitialPin] = useState<{lat: number, lng: number} | null>(null);
  ...
  // isCalculated means it is Confirmed and locked
  const [isCalculated, setIsCalculated] = useState(false);
  const [drivingDistanceKm, setDrivingDistanceKm] = useState<number | null>(null);
  // mapMountKey: increment this ONLY when we want to jump the map to a new center.
  // Using defaultCenter (uncontrolled) + key re-mount lets user freely pan/zoom
  // without the map snapping back on every render.
  ...
  useEffect(() => {
    if (user) {
      supabase.from('customer').select('address, phone, delivery_lat, delivery_lng, calculated_distance_km').eq('id', user.id).single().then(({ data }) => {
        if (data) {
          if (data.address) {
          ...
          if (data.delivery_lat && data.delivery_lng) {
            const loc = { lat: data.delivery_lat, lng: data.delivery_lng };
            setPinLocation(loc);
            setMapCenter(loc);
            setMapMountKey(k => k + 1); // re-mount map centered on saved pin
            setInitialPin(loc);
            
            if (data.calculated_distance_km !== null && data.calculated_distance_km !== undefined) {
              setInitialDistance(data.calculated_distance_km);
              setDrivingDistanceKm(data.calculated_distance_km);
              
              // Automatically lock and calc fee without calling Ola API
              setIsCalculated(true);
              calculateFeeOnly(data.calculated_distance_km);
            }
          }
          // If no saved location: pin stays at cafe default, user can move it
        }
      });
    }
  }, [user]);
  ...
  useEffect(() => {
    if (orderType !== 'delivery') {
      ...
      if (initialDistance !== null && initialPin) {
        setIsCalculated(true);
        setDrivingDistanceKm(initialDistance);
        calculateFeeOnly(initialDistance);
      } else {
  ...
  const calculateFeeOnly = async (distKm: number) => {
    setLoading(true);
    ...
    if (!error && data && !data.error) {
      ...
      showAlert("Error", "Failed to calculate delivery fee.");
    }
    setLoading(false);
  };
  ...
  const handleUseCurrentLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        setPinLocation(loc);
        setMapCenter(loc);
        setMapMountKey(k => k + 1); // jump map to current location, then user can pan freely
        setLoading(false);
      },
      (_error) => {
        setLoading(false);
        showAlert("Location Error", "Unable to retrieve your location.");
      },
  ...
  const calculateDistanceAndFee = async () => {
    ...
    // Validate phone using regex (Indian format typically 10 digits)
    ...
    // Check if location pin changed
    const pinChanged = !initialPin || pinLocation.lat !== initialPin.lat || pinLocation.lng !== initialPin.lng;
    
    let distKm = drivingDistanceKm || 0;
    
    if (pinChanged || drivingDistanceKm === null) {
      try {
      ...
      setInitialPin(pinLocation);
      setInitialDistance(distKm);
    }
    
    // Update Supabase customer table
    if (user) {
  ...
  const handleCheckout = async () => {
    ...
    setLoading(true);
    try {
      // Build cart payload — server will verify prices independently
      ...
      // Step 1: Create Razorpay order on server. NO DB writes happen yet.
      const { data: rzpOrder, error: rzpError } = await supabase.functions.invoke('create-razorpay-order', {
        body: {
          customer_id: user.id,
          items,
          order_type: orderType,
          business_type: (cart[0]?.item as any)?.business_type || 'cafe',
      ...
      if (rzpError) throw new Error('Failed to initiate payment. Please try again.');
      ...
      const options = {
        ...
        handler: async function (response: any) {
          // Step 2: Verify payment on server. DB writes (order, items, payment) happen HERE.
          const { data: verifyData, error: verifyError } = await supabase.functions.invoke('verify-razorpay-payment', {
            body: {
              razorpay_order_id: response.razorpay_order_id,
              razorpay_payment_id: response.razorpay_payment_id,
              razorpay_signature: response.razorpay_signature,
              pre_order_id: rzpOrder.pre_order_id,
              verified_payload: rzpOrder.verified_payload,
            }
          });

          if (verifyError) {
            showAlert("Payment Error", verifyError.message || "Verification request failed.");
          } else if (verifyData?.error) {
            showAlert("Payment Failed", verifyData.error);
          } else {
            clearCart();
            navigate('/track/' + verifyData.order_id);
          }
        },
                ...
                <>
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {drivingDistanceKm !== null ? drivingDistanceKm.toFixed(1) + ' km' : 'Calculated next'}
                    </span>
                  </div>
                  <div className="flex justify-between">
          ...
          <div className="space-y-6">
            <div className="bg-white p-5 rounded-lg shadow-sm border space-y-5">
                  ...
                  <div className="pt-2">
                    <div className="flex justify-between items-end mb-2">
                      ...
                      <button 
                        ...
                        disabled={isCalculated}
                        className="text-xs flex items-center gap-1 bg-blue-50 text-blue-700 px-2 py-1 rounded hover:bg-blue-100 disabled:opacity-50 font-medium"
                      >
                    ...
                    <div className="h-64 bg-gray-100 rounded-lg overflow-hidden relative border">
                      <Map
                        ...
                        onClick={(ev: any) => {
                          if (!isCalculated) {
                            setPinLocation(ev.detail.latLng);
                            // intentionally NOT updating mapCenter here —
                            // user clicked, so they're already looking at this spot
                          }
                        }}
                      ...
                      >
                        <AdvancedMarker position={CAFE_LOC} title="Cafe Location">
                        ...
                        <AdvancedMarker
                          ...
                          onDragEnd={(ev: any) => {
                            // Only update pin — don't re-center, user is already looking here
                      ...
                      {!isCalculated && (
                        <div className="absolute top-2 left-2 right-2 bg-white/90 backdrop-blur text-xs p-2 text-center rounded shadow-sm text-gray-700 pointer-events-none">
                          Tap map to place pin, or drag the pin to set exact location
                        </div>
                      )}
                    </div>
                  </div>
              ...
              {!isCalculated ? (
                ...
                >
                  {loading ? 'Calculating...' : 'Confirm Details & Proceed'}
                </button>
              ) : (
                <button
